...
class DetermineComplexityTest : public ::testing::Test
{
  protected:
	...
	std::pair<std::vector<double>, std::vector<size_t>> generateTestData(Complexity expectedComplexity, size_t size = 3)
	{
		...
		for (size_t i = 0; i < size; ++i) {
			...
			case Complexity::ON2:
				...
				break;
			case Complexity::ON3:
				times[i] = static_cast<double>(input_sizes[i] * input_sizes[i] * input_sizes[i]);
				break;
			case Complexity::O2N:
...
{
	auto [times, input_sizes] = generateTestData(Complexity::ONFactorial,
...
TEST_F(DetermineComplexityTest, ComplexityInvalid)
{
	...
	std::vector<double> times       = {1.0, 2.0, std::nan("1"), -1.0}; // Deliberate NaN and negative values
...
TEST_F(DetermineComplexityTest, ComplexityWithNonPositiveValues)
{
	...
	std::vector<size_t> input_sizes = {1, 2, 0, 3}; // includes 0 which is non-positive
